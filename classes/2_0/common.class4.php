    ...
    function login($inputData){ // tp // done
		...
		function removeTrove($inputData){ // tp // done
				...
				if($isUpdated){
						...
						$updateQuery = " UPDATE ".TBL_TRADES." SET trd_status = '9' WHERE (FIND_IN_SET('".$troveId."',trd_purpose_trv_id_list) OR FIND_IN_SET('".$troveId."',trd_desire_trv_id_list)) ";
			  ...
			  $totalPurposedTrove = $this->db->getValue(TBL_TROVES,"count(trv_id)");
				...
				if($tradeId){
						...
						if(is_array($frdDeviceList) && count($frdDeviceList) > 0){
							...
							$message['aps']['alert'] 		= $senderData['u_first_name']." ".$senderData['u_last_name']." propose a trade, check it out in here!";
							$message['aps']['badge'] 		= "1";
		...
		function getUserList($inputData){ // tp // done 
				...
				$this->db->where('((rpt_u_id = ? AND  = ?) OR (rpt_other_u_id = ? AND rpt_u_id = ?))',array($userId,$otherUserId,$userId,$otherUserId))
						->where('','1');
			    $reportData = $this->db->getOne();
