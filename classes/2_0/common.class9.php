...
/*		function uploadChatMediaData($inputData){
	...
	function cronSendAPNSForRemainReviewAfterTrade(){
		set_time_limit(0);
		// Make new entries for reveiew to each other user after succussfully trade.
		$twoDayBefore = $this->utc_time - (60*60); //2*24*
		$isReviewTrue = "true";
		while($isReviewTrue == "true"){
			...
			$sql = "
				SELECT
					ou.*,udt.*
				FROM
					".TBL_TRADE_REVIEW_REMAINS."
				JOIN
					".TBL_USER." as u ON u.u_id = ruw_u_id AND u.u_status = '1' 
				JOIN
					".TBL_USER_DEVICE_TOKENS." as udt ON udt_u_id = ruw_u_id AND udt_device_token != '' AND udt_device_token != 'null'
				JOIN
					".TBL_USER." as ou ON ou.u_id = ruw_other_u_id AND ou.u_status = '1' 
				WHERE
			...
			if(checkArr($remainReviewData)){
						if($remainReviewData['0']['udt_device_type'] == "1" && strlen($remainReviewData['0']['udt_device_token']) > 32 ){
								try{
										$body = array();
										$body['aps']['alert']   = "Make Review for ".(trim($remainReviewData['0']['u_first_name']." ".$remainReviewData['0']['u_last_name'])); //"New message in event :"+$eid;
	...
	//  NOT IN USE AND NOT WHOLE WORKED DONE
