...
class Common{
	...
	function reportUser($inputData){
		...
		function unblockUser($inputData){
			...
			if(!checkArr($otherUserData)){
				$returnData = array(
						'statusCode'			=> "2",
						'message'			=> "This person's profile is no longer available for viewing.",
					);
				return $returnData;
			}
			
			$this->db->where('rpt_u_id',$userId)
					->where('rpt_other_u_id',$otherUserId)
					->where('rpt_status','1');
			...
			$this->db->where("(rpt_u_id = ? AND rpt_other_u_id = ?)",array($userId,$otherUserId));
			$this->db->delete(TBL_REPORTED_USERS);
		...
		function getUserProfile($inputData){ // tp // done
				...
				if(checkArr($reportData)){
					$returnData = array(
							'statusCode'		=> ($reportData["rpt_u_id"] == $userId) ? "-3" :"-2", // Kamal asked me to do this error code
							'message'			=> ($reportData["rpt_u_id"] == $userId) ? "You blocked this user. Would you like to unblock them and view their profile?" : "This person's profile is no longer available for viewing.",
							'isBlocked'			=> "true",
							'isBlockedByYou'	=> $reportData["rpt_u_id"] == $userId ? "true" : "false",
			...
			/*		if($reportData['rpt_u_id'] == $userId){
				...
				if(checkArr($data)){
						...
						$reportedIds = $this->db->subQuery();
						$reportedIds->where('rpt_trv_u_id',$userId)
							 ->where('rpt_trv_status','1');
						$reportedIds->get(TBL_REPORTED_TROVES,null,'rpt_trv_trv_id');
						
						$this->db->join(TBL_MATERIALS_AS,"trv_mtl_id = mtl_id AND mtl_status = 1","LEFT");
			//			$this->db->join(TBL_USERS_AS,"u_id = trv_u_id AND u_status = 1");
						$this->db->join(TBL_CATEGORIES_AS,"cat_id = trv_cat_id AND cat_status = 1");
						$this->db->where("trv_u_id",$otherUserId)
								 ->where("trv_id",$reportedIds,"NOT IN")
		...
		function getCategoryList($inputData){ // tp // done
				...
				if($type == '1' || $type == '0'){
					...
					if($userId != null){
						...
						$itemCategoryList = $this->db->get(TBL_CATEGORIES." as cat",null);
					}
					...
					foreach($itemCategoryList as $key => $value){
							...
							if($userId != null){
								...
								$subCategoryList = $this->db->get(TBL_CATEGORIES." as cat",null);
							}
				...
				if($type == '2' || $type == '0'){
					...
					if($userId != null){
						...
						$serviceCategoryList = $this->db->get(TBL_CATEGORIES." as cat",null);
					}
		...
		function updateLastTradeSeen($inputData){
			...
			$badgeCount			= $inputData['badgeCount'];
			...
			if($badgeCount != ""){
				$updateData['u_badge_count'] = $badgeCount;
			}
		...
		function getTroveDetail($inputData){ // tp // done
                ...
                function removeTroveTradeBanner($inputData){ // tp // done
                    $userId 				= $inputData['userId'];
                    $troveId				= $inputData['troveId'];
                    ...
                    if(!checkArr($troveDetail)){
                        $returnData = array(
                                        'statusCode' => "2",
                                        'message'	 => "Sorry, This trove is not available longer.",
                                );
                        return $returnData;
                    }

                    $troveUpdateData = array(
                                'tv_after_trade_expire_date'	=> "0",
                                'trv_modified_date'	=> $this->utc_time,
                            );
                    $this->db->where('trv_id',$troveId)
                                     ->where('trv_status',"1");
						...
						if(is_array($frdDeviceList) && count($frdDeviceList) > 0){
							...
							foreach($frdDeviceList as $key=> $value){
								if($value['udt_device_token'] != ""){
									$totalBadge = $value['u_badge_count'];
							...
							$message['aps']['badge'] 		= $totalBadge + 1;
		...
		function moderateTrade($inputData){ // tp // done  
				...
				if($isUpdated){
						if($status == '1'){
								...
								$troveUpdateData = array(
                                                                        // we have removed logic for traded trove will be removed after 48 hours. 
									//	'tv_after_trade_expire_date'	=> $this->utc_time + (2*24*60*60),
                                                                                    'tv_after_trade_expire_date'	=> $this->utc_time,
                                                                                    'trv_modified_date'	=> $this->utc_time,
									);
			  ...
			  $remainReview = $this->db->getOne(TBL_TRADE_REVIEW_REMAINS);
				...
				if($reviewId){
						...
						if(is_array($frdDeviceList) && count($frdDeviceList) > 0){
							...
							$message = array();
							$message['aps']['icon'] 		= "appicon";
							$message['aps']['alert'] 		= $senderData['u_first_name']." ".$senderData['u_last_name']." just reviewed you. See what they wrote!";
							$message['aps']['badge'] 		= $totalBadge + 1;
							$message['aps']['sound'] 		= "default";
							$message['aps']['uname'] 		= $senderData['u_first_name']." ".$senderData['u_last_name'];
							$message['aps']['uid'] 			= $userId;
							$message['aps']['type']			= "reviewmade";
							$message['aps']['rvwId']		= $reviewId;
		...
		function getBrowseTroveList($inputData){ // tp // done 
			...
			//	$orderByQuery = "trv_id DESC";
				...
				if(empty($userData)){
					$userData['u_latitude'] = $inputData['latitude'];
					$userData['u_longitude'] = $inputData['longitude'];
				}
				if($pageNo == '1'){
								...
								WHERE
										...
										trv_id NOT IN (
												...
												FROM
													".TBL_REPORTED_TROVES."
												WHERE
														rpt_trv_u_id = '".$userId."'AND rpt_trv_status = '1'
											) AND 
										trv_status = '1'
								".$havingQuery."
						";
						...
						if(checkArr($countData)){
                                ...
                                //tv_after_trade_expire_date = '0' AND
				$qry = "SELECT
										".$this->trvFL.",".$this->catFL.",".$this->uFL.",IF(u_dsr_id > 0,1,0) as desireSorting,get_distance_in_miles_between_geo_locations('".$userData['u_latitude']."','".$userData['u_longitude']."',u_latitude,u_longitude) as distance 
								FROM
										".TBL_TROVES_AS."
								JOIN
										".TBL_USERS_AS." ON u_id = trv_u_id and u_status = '1'
								JOIN
										".TBL_CATEGORIES_AS." ON cat_id = trv_cat_id AND cat_status = 1
								LEFT JOIN
										".TBL_USER_DESIRES." ON u_dsr_u_id = '".$userId."' AND u_dsr_cat_id = cat_id AND u_dsr_status = 1
								WHERE
										".$whereQuery."
										trv_u_id != '".$userId."' AND
										trv_u_id NOT IN (
								...
								".$havingQuery."
								ORDER BY
										desireSorting DESC, ".$orderByQuery."
								LIMIT
										".$pageStartLimit.", ".$limit."
					";		
				$dataList = $this->db->rawQuery($qry,null,false);
		...
		/*			if($sent == 1){
			...
			if(checkArr($remainReviewData)){
						if($remainReviewData['0']['u_id'] > "0"  && $remainReviewData['0']['udt_device_type'] == "1" && $remainReviewData['0']['udt_device_token'] != "" && strlen($remainReviewData['0']['udt_device_token']) > 32 ){
								try{
										$totalBadge = $remainReviewData['0']['u_badge_count'];
										
										$body = array();
										$body['aps']['alert']   = "Help create a safe environment. Please review ".(trim($remainReviewData['0']['u_first_name']." ".$remainReviewData['0']['u_last_name'])).", with whom you recently traded.";
