...
class Common{
    ...
    public function __construct()
    {
				...
				$this->trdFL								= "trd.*";
				$this->uRvwFL								= "uRvw.*";
				$this->catFL								= "cat.cat_id,cat.cat_name,cat.cat_parent_cat_id,cat.cat_photo,cat.cat_status";
    ...
    function login($inputData){ // tp // done
		...
		function getUserProfile($inputData){ // tp // done
				...
				$this->db->where('u_id',$otherUserId)
						 ->where('u_status','1');
				$data = $this->db->getOne(TBL_USERS." as u",$this->uMyFL.",IF(rv_remain_other_u_id = u_id,1,0) as isReviewRemain"); 
					
				if(checkArr($data)){
						
						$this->db->join(TBL_CATEGORIES." as cat","cat_id = u_dsr_cat_id AND cat_status = 1");
						$this->db->where("fav_cat_u_id",$otherUserId);
						$desireList = $this->db->get(TBL_USER_DESIRES." as uDsr",null,$this->catFL.",".$this->uDsrCatFL);
						
						foreach($favCategoryList as $key => $value){
								$desireList[$key]['categoryImage'] = categoryImage($value['cat_id'],$value['cat_photo']);
						}
		...
		function getCategoryList($inputData){ // tp // done
				...
				$this->db->where("cat_type","1")
								 ...
								 ->where("cat_status","1");
				$categoryList = $this->db->get(TBL_CATEGORIES." as cat",null,$this->catFL.",".$this->uDsrCatFL);
				
				foreach($categoryList as $key => $value){
						$categoryList[$key]['categoryImage'] = categoryImage($value['cat_id'],$value['cat_photo']);
						$this->db->join(TBL_USER_DESIRES." as uDsr","u_dsr_u_id = '".$userId."' AND u_dsr_cat_id = cat_id AND u_dsr_status = 1","LEFT");
						$this->db->where("cat_type","1")
						...
						foreach($subCategoryList as $subKey => $subValue){
								$subCategoryList[$subKey]['categoryImage'] = categoryImage($subValue['cat_id'],$subValue['cat_photo']);
						}
		...
		function getUserTroveList($inputData){ // tp // done
				...
				$this->db->join(TBL_MATERIALS,"trv_mtl_id = mtl_id AND mtl_status = 1");
				$this->db->where("trv_u_id",$otherUserId)
								 ->where("trv_status","1");
				$this->db->orderBy("trv_id","desc");
	...
	//			 ->where('udt_security_token',$securityToken)
		...
		$this->db->where('frd_u_id',$userId)
				 ->where('frd_other_u_id',$ids,'NOT IN')
				 ->where('frd_from',"2");
		$frdDeleted = $this->db->delete(TBL_FRIENDS);
		...
		$this->db->where('frd_other_u_id',$userId)
						 ->where('frd_u_id',$ids,'NOT IN')
						 ->where('frd_from',"2");
		$frdDeleted = $this->db->delete(TBL_FRIENDS);
		
		// GET NEW FRIENDS UPDATED IN FACEBOOK
		$frdIds = $this->db->subQuery();
		$frdIds->where('(frd_u_id = ? OR frd_other_u_id = ?)',array($userId,$userId));
				   //->Where('frd_from','2');
		$frdIds->get(TBL_FRIENDS,null,"IF(frd_u_id = '".$userId."',frd_other_u_id,frd_u_id) as u_id");
		...
		foreach($newFriednList as $key => $value){
			$newFrdData = array (
				"frd_u_id" 					=> $userId,
				"frd_other_u_id" 	  => $value['u_id'],
				"frd_from" 				  => "2",
				"frd_created_date" 	=> $this->utc_time,
				"frd_status" 				=> "1",
			);
			$id = $this->db->insert (TBL_FRIENDS, $newFrdData);
