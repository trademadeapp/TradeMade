<?php
    // Jay Guru Maharaj
    // START:17-12-2015
    require_once('../../config/2_0/config.php');
    
    // COMMON FUNCTION AND LIBRARIES INCLUDED IN COMMON.PHP
    require_once('../../config/2_0/common.php');
  
    includeClasses('2_0/common');
    $common     = new Common();
    ...
    // $my_file = date('Y-m-d-H-i-s-').rand(100,999).'.json';
    // $my_file = fopen($my_file,'w');
    $logfile = [
        'get' => $_GET,
        'post' => $_POST,
        'file' => $_FILES,
    ...
    // fwrite($my_file,json_encode($logfile));
    switch($api_function)
    {
        ...
        case 'testPush':
            $token          = getPostVar('token');
            $body = array();
            $body['aps']['alert']   = 'push from code'; //"New message in event :"+$eid;
            $body['aps']['uid'] 	= 0;
            $body['aps']['type'] 	= "1";
            $body['aps']['badge'] 	= 1;
            ...
            $message['aps']['alert'] 		= "First Name LAst Name countered your trade, check out their offer!";
            $message['aps']['badge'] 		= 1;
            $message['aps']['sound'] 		= "default";
            $message['aps']['uname'] 		= "Name";
            $message['aps']['uid'] 			= 12;
            $message['aps']['type']			= "countertrade";
            $message['aps']['trdId']		= 13;

            

            $sendNotification 		= send_notification_ios($token,$message);
